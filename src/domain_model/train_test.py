...
def optimal_threshold(y_true, y_score, mapping_dom2types, cut_off):
    ...
    result = brute(macro_f1, args=(y_true, y_score, mapping_dom2types, cut_off), ranges=bounds, full_output=True, Ns=20, workers=3)
...
def run_domain_model(args):
    ...
    # type2id = load_vocab_dict(os.path.join(VOCAB_DIR_DICT[args.d_goal], DATA_DIR_DICT[args.dataset], "types_" + args.dfn_postfix + ".txt"))
    args.label_num = len(type2id)

    # mapping_dom2types = np.load(os.path.join(VOCAB_DIR_DICT[args.d_goal], DATA_DIR_DICT[args.dataset], 'dom2types_' + args.dfn_postfix + '.npy'))
    ...
    cn_mapping = None
    if args.cn:
        cn_mapping = np.load(os.path.join(VOCAB_DIR_DICT[args.d_goal], DATA_DIR_DICT[args.dataset], 'cn2org_wanli.npy'))
